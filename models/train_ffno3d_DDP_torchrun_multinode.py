...
class Trainer:
    ...
    # Utility function to gather losses from all processes
    def gather_losses(self, loss):
        loss_tensor = torch.tensor([loss], device=self.local_rank)
    ...
    # Utility function to broadcast the learning rate
    def broadcast_lr(self, optimizer):
        for param_group in optimizer.param_groups:
            lr_tensor = torch.tensor([param_group['lr']], device=self.local_rank)
    ...
    def train(self, name_config: str):
        ...
        for ep in range(self.epochs_run, epochs):
            ...
            for _ in self.train_data:
                a = _[0].to(self.local_rank)
                uE = _[1].to(self.local_rank)
                uN = _[2].to(self.local_rank)
                uZ = _[3].to(self.local_rank)
            ...
            gathered_relative_train_losses = self.gather_losses(train_losses_relative.avg)
            gathered_absolute_train_losses = self.gather_losses(train_losses_absolute.avg)

            # Compute global average
            global_relative_train_loss = sum(gathered_relative_train_losses) / len(gathered_relative_train_losses)
            global_absolute_train_loss = sum(gathered_absolute_train_losses) / len(gathered_absolute_train_losses)

            if dist.get_rank() == 0:
                    train_history['loss_relative'].append(global_relative_train_loss.item())
                    train_history['loss_absolute'].append(global_absolute_train_loss.item())
            ...
            with torch.no_grad():
                ...
                # Gather losses from all processes
                gathered_relative_losses = self.gather_losses(val_losses_relative.avg)
                gathered_absolute_losses = self.gather_losses(val_losses_absolute.avg)

                # Compute global average
                global_relative_loss = sum(gathered_relative_losses) / len(gathered_relative_losses)
                global_absolute_loss = sum(gathered_absolute_losses) / len(gathered_absolute_losses)

                if dist.get_rank() == 0:
                    val_history['loss_relative'].append(global_relative_loss.item())
                    val_history['loss_absolute'].append(global_absolute_loss.item())

                    # Step the learning rate scheduler on the main process
                    lr_scheduler.step(global_relative_loss.item())

                # Broadcast the learning rate to all processes
                self.broadcast_lr(self.optimizer)

                t2 = timeit.default_timer()
                print(f'Epoch {ep+1}/{epochs}: {t2-t1:.2f}s - Training loss = {global_relative_train_loss.item():.5f} - Validation loss = {global_relative_loss.item():.5f}'\
                    f' - Training accuracy = {global_absolute_train_loss.item():.5f} - Validation accuracy = {global_absolute_loss.item():.5f}')

                # save the model
                if global_relative_loss.item() < best_loss and dist.get_rank() == 0:
                    best_loss = global_relative_loss.item()
                    ckp = self.model.module.state_dict() #DDP
                    torch.save(ckp, './logs/models/bestmodel-'+name_config+f'-epochs{epochs}.pt')
                
                if early_stopper.early_stop(global_relative_loss.item()):
                    break

                # save intermediate losses
                if ep%2==0 and dist.get_rank() == 0:
                    ...
                    if ep>=2 and os.path.exists(f'./logs/loss/loss-{name_config}-epoch{ep-2}on{epochs}.h5'):
